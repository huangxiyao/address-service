...
database.sh initializeInventory "${companyCode}" "${environmentCode}" ${inventoryHosts}

deploymentHosts="$(database.sh deploymentInventory ${companyCode} ${environmentCode})"
...
then
    ...
    if (( failed == 0 ))
    then
        database.sh deploymentSucceeded "${displayName}" ${companyCode} ${environmentCode} ${playbookVersion} ${platformVersion} ${userName}
    else
        database.sh deploymentFailed "${displayName}" ${companyCode} ${environmentCode} ${playbookVersion} ${platformVersion} ${userName} $(successfulHosts)
    fi
fi
