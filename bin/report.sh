...
function gitLogTable
{
    ...
    echo "${changes}" | while read commit author subject
    do
...
function historyTable
{
    ...
    local deploymentIdentifier displayName time companyCode environmentCode platformVersion playbookVersion userName success

    database.sh deploymentHistory | head --lines 100 |
    while read deploymentIdentifier displayName time companyCode environmentCode playbookVersion platformVersion userName success
    do
        ...
        local previousPlatformVersion previousPlaybookVersion
        read previousPlatformVersion previousPlaybookVersion <<< "$(database.sh previousPlatformAndPlaybookVersions ${companyCode} ${environmentCode} ${time})"
...
function inventoryStatus {
    ...
    local rows="$(database.sh deploymentHostsSuccess ${companyCode} ${environmentCode})"
    [[ -z "${rows}" ]] && return
        ...
        for (e = 1; e <= environmentCount; ++e) {
            ...
            for (c = 1; c <= companyCount; ++c) {
                if ((orderedCompanies[c],orderedEnvironments[e],1) in hosts) {
                    ...
                    command = sprintf("inventoryStatus %s %s", orderedCompanies[c], orderedEnvironments[e])
...
function doSearch(value) {
    ...
    for (var r = 0; r < rows.length; ++r) {
        var row = rows[r];
        row.style.display = termsInRow(terms, row) ? "" : "none";
    }
}
function termsInRow(terms, row) {
    row.normalize();
    var termsMatched = true;
    for (var t = 0; t < terms.length && termsMatched; ++t) {
        termsMatched = termInRow(terms[t], row);
    }
    return termsMatched;
}
function termInRow(term, row) {
    var cells = row.childNodes;
    var termMatched = false;
    for (var c = 0; c < cells.length && ! termMatched; ++c) {
        termMatched = termInCell(term, cells[c]);
    }
    return termMatched;
}
function termInCell(term, cell) {
    return cell.firstChild.textContent.toLowerCase().startsWith(term);
}
</script>
</head><body>
<input id="search" type="search" autofocus placeholder="Search" oninput="doSearch(this.value)">
$(historyTable)
$(installationTable)
</body></html>
END_HTML
