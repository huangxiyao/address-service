...
import com.hp.it.cas.match.address.rest.LooselyValidatedAddressFinderRestProxy;

/**
 * This class acts as a client to invoke AddressDoctor standardization rest
 * interface process the address defined by yourself with LooselyAddress way.
 * 
 * @see {@link com.hp.it.cas.match.address.rest.LooselyValidatedAddressFinderRestProxy}
 * @version 1.0
 * @author Hu, Fei-Yang (Edward, HPIT-DS-CDC)
 * 
 */
public class LooselyAddressFinderRestProxyClient {
	 ...
	 */
	public final static AddressQueryResult executeLooselyAddressAccess(String addressQueryResultSuitID) {
		LooselyValidatedAddressFinderRestProxy looselyAddressFinderRestProxy = null;
		...
		try {
			...
			if (!AddressValidatedClientSupport.DIRECT_URL_ACCESS_FLAG
				...
				looselyAddressFinderRestProxy = new LooselyValidatedAddressFinderRestProxy(
						AddressValidatedClientSupport
								.getAddressDoctorURL(AddressValidatedClientSupport.LOOSEVALIDATED_ACCESS_URL_CODE));
			} else {
				looselyAddressFinderRestProxy = new LooselyValidatedAddressFinderRestProxy(
		...
		AddressQueryResult result = looselyAddressFinderRestProxy.find(addressQueryResultSuit.getAddressQuery());
		return result;
	}
	...
	public static boolean compareExpectedWithActual(String AddressQueryResultSuitID) {
		...
		try {
			addressQueryResultSuit = AddressValidatedClientSupport.getAddressQueryResultSuitMap().get(
					AddressQueryResultSuitID);
			result = executeLooselyAddressAccess(addressQueryResultSuit.getAqsID());
			...
			expected = AddressValidatedClientSupport.getResultStringByResultCode(addressQueryResultSuit
					.getLooselyValidatedResultCode());
			String actualStatements = "Actual   AddressQueryResult with AddressQuery "
					+ addressQueryResultSuit.getAqsID() + " through LooselyAddress of rest way is " + actual;
			String expectedStatements = "Expected AddressQueryResult with AddressQuery "
					+ addressQueryResultSuit.getAqsID() + " through LooselyAddress of rest way is " + expected;
			...
			if (expected.equals(actual)) {
				...
				logger.fatal("The AddressQueryResult with AddressQueryID:" + AddressQueryResultSuitID
						+ " through LooselyAddress of rest way is right!");
				return true;
			} else {
				logger.fatal(actualStatements);
				logger.fatal(expectedStatements);
				logger.fatal("The AddressQueryResult with AddressQueryID:" + AddressQueryResultSuitID
						+ "  through LooselyAddress of rest way is wrong!");
				return false;
			}
		} catch (Exception e) {
			logger.error(e.toString(), e);
