 ...
 * interface process the address defined by yourself with CertifiedAddress way
 * 
 ...
 * 
 */
public class CertifiedAddressFinderRestProxyClient {
	 ...
	 */
	public final static AddressQueryResult executeCertifiedAddressAccess(String addressQueryResultSuitID) {
		CertifiedAddressFinderRestProxy certifiedAddressFinderRestProxy = null;
		...
		try {
			...
			if (!AddressValidatedClientSupport.DIRECT_URL_ACCESS_FLAG
				...
				certifiedAddressFinderRestProxy = new CertifiedAddressFinderRestProxy(
						AddressValidatedClientSupport
								.getAddressDoctorURL(AddressValidatedClientSupport.CERTIFIEDADDRESS_ACCESS_URL_CODE));
			} else {
				certifiedAddressFinderRestProxy = new CertifiedAddressFinderRestProxy(
		...
		AddressQueryResult result = certifiedAddressFinderRestProxy.find(addressQueryResultSuit.getAddressQuery());
		return result;
	}
	...
	public static boolean compareExpectedWithActual(String AddressQueryResultSuitID) {
		...
		try {
			addressQueryResultSuit = AddressValidatedClientSupport.getAddressQueryResultSuitMap().get(
					AddressQueryResultSuitID);
			result = executeCertifiedAddressAccess(addressQueryResultSuit.getAqsID());
			...
			expected = AddressValidatedClientSupport.getResultStringByResultCode(addressQueryResultSuit
					.getCertifiedAddressResultCode());
			String actualStatements = "Actual   AddressQueryResult with AddressQuery "
					+ addressQueryResultSuit.getAqsID() + " through CertifiedAddress of rest way is " + actual;
			String expectedStatements = "Expected AddressQueryResult with AddressQuery "
					+ addressQueryResultSuit.getAqsID() + " through CertifiedAddress of rest way is " + expected;
			...
			if (expected.equals(actual)) {
				...
				logger.fatal("The AddressQueryResult with AddressQueryID:" + AddressQueryResultSuitID
						+ " through certifiedAddress of rest way is right!");
				return true;
			} else {
				logger.fatal(actualStatements);
				logger.fatal(expectedStatements);
				logger.fatal("The AddressQueryResult with AddressQueryID:" + AddressQueryResultSuitID
						+ "  through certifiedAddress of rest way is wrong!");
				return false;
			}
		} catch (Exception e) {
			logger.error(e.toString(), e);
