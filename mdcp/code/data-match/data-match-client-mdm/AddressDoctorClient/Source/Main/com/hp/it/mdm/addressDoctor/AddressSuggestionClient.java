...
public class AddressSuggestionClient extends AddressDoctorClient {
    ...
    public static final Logger log = Logger.getLogger(AddressSuggestionClient.class);
    ...
    public void cleanse(Map context, Map input, Map output) {
        ...
        if (!allGood)
            return;
        String server_url = cleanseProps.getProperty("AddressSuggestionsAddressFinderRestProxy_URL");
        AddressSuggestionsAddressFinderRestProxy proxy = new AddressSuggestionsAddressFinderRestProxy(server_url);
        /* get map inputs */
        String namein = (String) input.get(AddressDoctorInputs.NAMEIN);

        AddressQueryResult result = proxy.suggest(new AddressDoctorQuery().addressQuery(input));
