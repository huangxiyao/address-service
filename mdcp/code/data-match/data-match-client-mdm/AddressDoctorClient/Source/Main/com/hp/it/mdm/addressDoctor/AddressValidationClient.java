...
public class AddressValidationClient extends AddressDoctorClient {
    ...
    public static final Logger log = Logger.getLogger(AddressValidationClient.class);
    ...
    public void cleanse(Map context, Map input, Map output) {
        ...
        if (!allGood)
            return;
        String server_url = cleanseProps.getProperty("ValidatedAddressFinderRestProxy_URL");
        ValidatedAddressFinderRestProxy proxy = new ValidatedAddressFinderRestProxy(server_url);
        /* get map inputs */
