...
public class Worker extends Thread {
	...
	String configFile = Worker.class.getResource("/cleanse.properties")
			.getPath();
	String inputFile = Worker.class.getResource("/inputs.txt").getPath();
	String outputFile = Worker.class.getResource("/outputs.txt").getPath();
	...
	private void getFunction() {
		...
		if (func == null) {
			System.err.println("There is no function by the names " + funcName
					+ " in Library " + lib.getLibraryDescriptor().getName());
			System.exit(1);
		}
	}
	...
	/**
	 * Populates inputs.txt with the input names for the current function
	 */
	private void populateFileInputNames() {
		try {
			...
			BufferedWriter bWrite = new BufferedWriter(new OutputStreamWriter(
					fos, "UTF8"));
			System.out
					.println("Populating input names in inputs.txt automatically...");
			...
			System.out
					.println("Please populate inputs.txt with input values and rerun");
			errorWrapUp();
	...
	/**
	 * process inputs.txt, cleanse the data and populate outputs.txt
	 */
	private void performCleanse() {
		...
		try {
			...
			while (curLine != null) {
				...
				// print the outputs to a file only print out every 100th to
				// reduce volume
				// if (numCalls % 100 == 0){
	...
	/**
	 * Makes sure that the input names in the input file are what is expected by
	 * the function
	 */
		...
		for (int i = 0; i < fileInputNames.length; i++) {
			...
			if (!found) {
				System.err
						.println("Input name "
								+ fileInputNames[i]
								+ " from the inputs.txt is not found in the function descriptor for "
								+ funcName);
				fileInputPtr[i] = -1;
			}
	...
	private void getInputOutputNames() {
		...
		Iterator iter = lib.getLibraryDescriptor().getFunctionDescriptors()
				.iterator();
	...
	private void populateOutputsWithNull(Map outputMap) {
		...
		String[] fieldsToGather = { "Building_COMPLETE_ALL",
				"CountryCode_ISO_3", "Locality_ALL", "POBox_COMPLETE",
				"PostalCode_FORMATTED", "Province_ABBREVIATION",
				"Street_COMPLETE_ALL", "ValidationStatus",
	...
	private final Object convertToType(int typePtr, String inValue) {
		...
		if (adapterInputs[typePtr].getType().equals(ParameterTypes.DATE)) {
			...
			try {
				...
				log.warn("Could not format Date " + inValue
						+ " to a date returning current date");
				return new Date();
			}
		}
		...
		try {
			for (int i = 0; i < adapterOutputNames.length; i++) {
				...
				if (curVal == null) {
					...
					logdebug("field name '" + adapterOutputNames[i]
							+ "' value '" + curVal + "'");
				}
	...
	private void handleError(String action, Exception e) {
		System.err
				.println("Problem " + action + ": " + e.getLocalizedMessage());
		e.printStackTrace();
		errorWrapUp();
		allGood = false;
	}
	...
	private void errorWrapUp() {
		...
		try {
			...
			System.err.println("Problem closing buffered reader or writer: "
