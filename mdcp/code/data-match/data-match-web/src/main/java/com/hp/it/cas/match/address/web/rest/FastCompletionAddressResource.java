...
@Path("fastCompletionAddress")
public class FastCompletionAddressResource extends AbstractAddressResource {
	private final Logger logger = LoggerFactory.getLogger(FastCompletionAddressResource.class);
	private final FastCompletionAddressFinder finder;

	public FastCompletionAddressResource(FastCompletionAddressFinder finder, MessageInterpolator messageInterpolator, Locale[] locales) {
		this.finder = finder;
	...
	public Response handleDocumentation(@Context ServletContext servletContext) throws MalformedURLException, URISyntaxException{
		logger.debug("ENTRY {}");
		return Response.ok().entity(servletContext.getResourceAsStream("/WEB-INF/fastCompletionAddressResourceDocumentation.html")).build();
	}
}
